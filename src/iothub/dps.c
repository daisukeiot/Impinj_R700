...
CLIENT_LL_HANDLE ProvisionDeviceX509(const char* scopeId, const char* modelId, APP_CONTEXT* appContext)
{
    ...
    if (dpsContext.registration_complete == 1)
    {
        ...
        else
        {
            ...
            if (appContext->iothub_uri != NULL)
            {
                free(appContext->iothub_uri);
            }
            (void)mallocAndStrcpy_s(&appContext->iothub_uri, dpsContext.iothub_uri);
        }
    }
