...
int curlStreamSpawnReaderThread(
    CURL_Stream_Session_Data* session_data)
{
    return ThreadAPI_Create(&session_data->threadData.StreamThreadHandle, curlStreamReader, session_data);
}
    ...
    CURL_Stream_Session_Data* session_data)
{
    ...
    if (ThreadAPI_Join(session_data->threadData.StreamThreadHandle, &res) != THREADAPI_OK)
    {
